...
# p grid: coarse + fine near transition
P_LIST=(0.40 0.45 0.50 0.55 0.60 0.65 0.70 0.75 0.80 0.58 0.62)

# Convergence study (only for these p)
P_CONV=("0.58" "0.60" "0.62")
M_CONV=(50 100 200 400)
...
for M in "${Ms[@]}"; do
  ...
  # -----------------------
  # IMPORTANT: adjust this line if your CLI differs
  # Pattern you gave: mpirun --bind-to none -np $SLURM_NTASKS ./forest_fire ...
  # Here we assume args: N p M seed
  # -----------------------
...
# Parse key,value pairs from program output (your log looks like: avg_steps,12.29,prob_reach_bottom,0,...)
...
{
  echo "N,p,M,seed,tasks,avg_steps,prob_reach_bottom,total_time_s,wall_time_s"
  echo "${N},${p},${M},${seed},${SLURM_NTASKS},${avg_steps},${prob_rb},${total_time},${wall}"
} > "$run_csv"
